...
public final class IncorporeticSkytouchingRecipes {
	...
	public static void init() {
		cygnusSpark = register(CygnusNetworkItems.CYGNUS_SPARK, new ItemStack(ModItems.corporeaSpark, 1, 0));
		masterCygnusSpark = register(CygnusNetworkItems.MASTER_CYGNUS_SPARK, new ItemStack(ModItems.corporeaSpark, 1, 1));
		
		cygnusWord = register(CygnusNetworkItems.WORD, ModBlocks.corporeaInterceptor);
		cygnusFunnel = register(CygnusNetworkItems.FUNNEL, ModBlocks.corporeaFunnel);
		cygnusRetainer = register(CygnusNetworkItems.RETAINER, ModBlocks.corporeaRetainer);
		cygnusCrystalCube = register(CygnusNetworkItems.CRYSTAL_CUBE, ModBlocks.corporeaCrystalCube);
	}
